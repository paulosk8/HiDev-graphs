...
- **UI**: sección transversal **«Repaso»** (`ModoEstudioPage`, sidebar con badge de pendientes de hoy) → sesión de recuerdo activo (mostrar respuesta → valorar No lo sé/Difícil/Bien/Fácil). Helper `lib/repaso.ts` (`pendientesHoy`, `tocaHoy`, `colorDominio`, `etiquetaDominio`). En el **Mapa**, toggle **«🎯 Dominio»** colorea los conceptos por dominio (gris = sin repasar, rojo→verde) con su leyenda. Verificado por 2 smokes (lógica SM-2/persistencia/reindexado y GUI de la sesión + badge + toggle).
