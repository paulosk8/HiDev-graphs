...
- **Página de Configuración** (`ConfiguracionPage`, sección `configuracion`): sub-navegación vertical a la izquierda + contenido a la derecha. Agrupa lo que antes colgaba del pie del sidebar: **Apariencia** (modo claro/oscuro, control principal), **Asistente IA** (la antigua `AsistentePage`, embebida tal cual), **Datos y copias** (Sincronizar, Actualizar, Copia de seguridad, Restaurar — cada uno con descripción) y **Cuenta** (usuario + cerrar sesión). El pie del sidebar queda con un acceso rápido al tema, la entrada ⚙️ Configuración y el chip de usuario. Verificado por smoke de GUI.
